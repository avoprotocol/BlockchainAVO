...
pub fn load_config(config_path: Option<&str>) -> Result<NodeConfig, Box<dyn std::error::Error>> {
    let config = match config_path {
        Some(path) => {
            println!("\x1b[33m[INFO]\x1b[0m Loading configuration from: {}", path);
            let config_content = std::fs::read_to_string(path)?;
            let config: NodeConfig = toml::from_str(&config_content)?;
            println!("\x1b[32m[SUCCESS]\x1b[0m Configuration loaded successfully");
            config
        }
        None => {
            println!("\x1b[33m[INFO]\x1b[0m Using default configuration");
            NodeConfig::default()
        }
    };
...
pub fn save_config(config: &NodeConfig, path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\x1b[32m[SUCCESS]\x1b[0m Configuration saved to: {}", path);
    Ok(())
}
...
pub fn validate_config(config: &NodeConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if config.network_port < 1024 {
        println!("\x1b[33m[WARNING]\x1b[0m Network port {} may require privileges", config.network_port);
    }
    
    if config.rpc_port < 1024 {
        println!("\x1b[33m[WARNING]\x1b[0m RPC port {} may require privileges", config.rpc_port);
    }
    
    // Check data directory
    if !config.data_dir.exists() {
        println!("\x1b[33m[INFO]\x1b[0m Creating data directory: {:?}", config.data_dir);
        std::fs::create_dir_all(&config.data_dir)?;
    }
    
    // Validate validator configuration
    if config.is_validator && config.validator_key_path.is_none() {
        println!("\x1b[33m[WARNING]\x1b[0m Validator mode enabled but no key path specified");
    }
    ...
    println!("\x1b[32m[SUCCESS]\x1b[0m Configuration validation completed");
    Ok(())
}
...
fn create_optimized_config_for_system(base_config: &NodeConfig) -> NodeConfig {
    ...
    println!("\x1b[36m[AUTO-INIT]\x1b[0m Optimized for {} CPU cores", cpu_count);
    println!("\x1b[36m[AUTO-INIT]\x1b[0m Configuration: {} shards, {} max peers", 
...
"#;
        ...
        println!("\x1b[32m[SUCCESS]\x1b[0m Created getting started guide: {}", readme_path);
    }
...
"#;
            ...
            println!("\x1b[32m[SUCCESS]\x1b[0m Created start script: {}", script_path);
        }
    }
...
"#;
    ...
    if config.consensus.validators_per_shard < 3 {
        println!("\x1b[33m[WARNING]\x1b[0m Less than 3 validators per shard may compromise security");
    }
    
    println!("\x1b[32m[SUCCESS]\x1b[0m Configuration validation passed");
    Ok(())
}
